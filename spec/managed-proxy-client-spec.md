...
| D7 | **Update-download entitlement gating.** | Client-local updater policy; no proxy § (not a wire behavior). | Sparkle updates are entitlement-gated: `SparkleHelper` blocks downloads when `isSparkleEntitledToUpdates()` is false. The gate exists chiefly for perpetual-license classes. | Windows deliberately does **not** gate `tauri-plugin-updater` in v1 (CAR-2258, recorded default 2026-07-20). Rationale: subscription-only edition; unentitled installs cannot process files anyway (fail-closed licensing), update access delivers fixes to lapsed users without extending paid capability, and gating would couple the updater to the Phase A entitlement seam prematurely. Convergence: revisit when Phase B entitlement tokens land (CAR-2175 seam) if gating is ever wanted; the gate source would be `licensing/AccountState`, not the entitlement store. |
